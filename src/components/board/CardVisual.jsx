...
// Pure presentational shell for a kanban card — the product's actual card
// face, rendered from props alone. No store reads, no Supabase, nothing
// that touches the network or the browser at module scope, so this is safe
// to import from a Node-prerendered marketing page (see CardCard.jsx's
// wrapper — `Card.jsx` — for why that split exists).
//
// `card` carries the same fields the DB row does (title, description,
// priority, due_date, checklist, completed, icon, assignees, assignee_name,
// assignee_refs, board_id) — everything else the visual needs (resolved
// labels, presence, viewer's own profile, settings, interaction callbacks)
// arrives as separate props, because those come from stores in the app and
// from static content on marketing pages.
export default function CardVisual({
  ...
  ghostArmed,
  // The real card is a <button> (it's clickable to open CardDetailPanel).
  // It also contains its own nested <button>s (complete-toggle, checklist
  // expand). That's fine client-side-only — the app has no server-rendered
  // HTML to hydrate against — but <button> can't legally contain <button>,
  // and a marketing page that prerenders CardVisual DOES have to hydrate:
  // the browser's HTML parser silently closes the outer button early,
  // producing a DOM shape that doesn't match what React expects and
  // triggering a hydration-mismatch remount (visible as duplicated page
  // content on load). Marketing call sites pass `interactive={false}` to
  // render a <div> shell instead — same classes, same look, just a legal
