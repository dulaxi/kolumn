...
in `src/__tests__/{Button,Input,Menu,Popover,Tooltip}.test.jsx`. (The
button/input/menu/ghost-button decision mockups were pruned 2026-07-24; the
rules they encoded live on in this file and the components themselves.)
...
- **Motion: tokens + `transform`/`opacity` only (2026-07-28 motion pass).** Durations/easings live as `--dur-*`/`--ease-*` tokens in `@theme`; JS deferred-unmount timers mirror them via `src/constants/motion.js` (`OVERLAY_EXIT_MS`/`MODAL_EXIT_MS`) — keep the pair in sync. `Modal` animates via `data-state`/`data-animated` (opt out with `animated={false}` for bespoke animation, e.g. QuickAddBar's pill-bounce). Reduced motion is two-trigger: OS `prefers-reduced-motion` + the Settings → Accessibility `motion` preference stamped as `data-motion` on `<html>` (`src/utils/motion.js`); every new keyframe/animation class must be added to BOTH selector lists in index.css's reduced-motion guard, and JS-driven animation (sprite loops, dnd drop) must consult `useReducedMotion()`.
