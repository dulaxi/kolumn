...
const MODELS = {
  lite:     'veo-3.1-lite-generate-preview',   // $0.05/s 720p · $0.08/s 1080p
  fast:     'veo-3.1-fast-generate-preview',   // $0.10/s 720p · $0.12/s 1080p
  standard: 'veo-3.1-generate-preview',        // $0.40/s 720p/1080p
};
...
let dur = Number(args.find(a => /^\d+$/.test(a)) || 4);
const imgArg = args.find(a => a.startsWith('--image='));
const wantLoop = args.includes('--loop');
if (wantLoop && dur < 8) { console.log(`--loop (lastFrame) requires >=8s on Veo 3.1 — forcing durationSeconds=8 (was ${dur}).`); dur = 8; }
const modelKey = (args.find(a => a.startsWith('--model=')) || '--model=fast').split('=')[1];
const MODEL = MODELS[modelKey] || MODELS.fast;
const resArg = (args.find(a => a.startsWith('--res=')) || '--res=720').split('=')[1];
const resolution = resArg === '1080' ? '1080p' : resArg === '4k' ? '4k' : '720p';
if (!prompt) { console.error('Usage: node veo.mjs "<prompt>" <outName> [seconds] [--image=file] [--loop] [--model=lite|fast|standard] [--res=720|1080]'); process.exit(1); }
...
const body = { instances: [instance], parameters: { aspectRatio: '16:9', durationSeconds: dur, resolution } };

console.log(`Submitting ${MODEL} job (${dur}s, ${resolution}) …`);
